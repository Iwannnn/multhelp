    ...
    methods: {
      ...
      _modelChange(newVal, oldVal) {
        ...
        if (diffObj) {
          ...
          this.$emit(isValid ? 'success' : 'fail', {
            detail: isValid ? {
              trigger: 'change'
            } : {
              errors,
      ...
      _showAllErrors(errors) {
        Object.keys(this.newRules).forEach(k => {
      ...
      _showError(prop, error) {
        const formItem = this.formItems[prop];
...
export default {
  ...
  watch: {
    models: {
      handler: '_modelChange',
      immediate: true,
      deep: true
    },
    rules: {
      handler: '_rulesChange',
      immediate: true,
      deep: true
    }
  },
  ...
  beforeMount() {
    this.initRules();
    this.formValidator = new _formValidator.default(this.models, this.newRules);
  },
